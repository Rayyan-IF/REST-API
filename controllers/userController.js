...
export const getProfile = async (req, res) => {
  try {
    const userProfile = await database.query('SELECT email, first_name, last_name, profile_image FROM users WHERE email = $1', [req.email])

    if (userProfile.rows.length === 0) return res.status(404).json(createResponse(STATUS_CODE.NOT_FOUND, "Pengguna tidak ditemukan"))
    
    res.status(200).json(createResponse(STATUS_CODE.SUCCESS, "Sukses", userProfile.rows[0]))
  } catch (error) {
    console.error(error)
